...
class LoadMultipleRSImageFromFile(BaseTransform):
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        img2 = tif.imread(filename2)
        
        # Load MSI image (4 channels)
...
@TRANSFORMS.register_module()
class LoadMultipleRSImageAndScalingFromFile(BaseTransform):
    """Load two Remote Sensing mage from file.
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        # Scaling
        cut_percent = 2.0
        cut_value = np.percentile(img2, [cut_percent, 100.0-cut_percent], axis=(0, 1))
        img2 = np.clip(img2, cut_value[0], cut_value[1])  
        
        vv = (img2[...,0]-np.min(img2[...,0]))/(np.max(img2[...,0])-np.min(img2[...,0])) 
        vh = (img2[...,1]-np.min(img2[...,1]))/(np.max(img2[...,1])-np.min(img2[...,1])) 
        result1 = np.divide(vv, vh, out=np.zeros_like(vv), where=(vh != 0))
        result1 = (result1-np.min(result1))/(np.max(result1)-np.min(result1)) 

        result2 = np.divide(vh-vv, vh+vv, out=np.zeros_like(vv), where=(vh+vv != 0))
        result2 = (result2-np.min(result2))/(np.max(result2)-np.min(result2)) 

        result3 = (np.square(vv)+np.square(vh))/2
        result3 = np.sqrt(result3)
        result3 = (result3-np.min(result3))/(np.max(result3)-np.min(result3)) 
        img2 = np.stack([vv, vh, result1, result2, result3], axis=-1)
...
@TRANSFORMS.register_module()
class LoadMultipleRSImageAndScalingBothFromFile(BaseTransform):
    """Load two Remote Sensing mage from file.
